---
title: "Lesson 13: Hypothesis Testing 1: Single-sample mean"
...
2.  Determine if a single-sample mean is different than a population mean using a hypothesis test.
...
## Hypothesis tests we will learn
...
|:----------:|:----------:|:----------:|:----------:|:----------:|:----------:|
| 12 | 5.1 | Pop mean | $\mu$ | Sample mean | $\overline{x}$ |
| 13 | 5.2 | Pop mean of paired diff | $\mu_d$ or $\delta$ | Sample mean of paired diff | $\overline{x}_{d}$ |
| 14 | 5.3 | Diff in pop means | $\mu_1-\mu_2$ | Diff in sample means | $\overline{x}_1 - \overline{x}_2$ |
| 16 | 8.1 | Pop proportion | $p$ | Sample prop | $\widehat{p}$ |
| 16 | 8.2 | Diff in pop prop's | $p_1-p_2$ | Diff in sample prop's | $\widehat{p}_1-\widehat{p}_2$ |
...
## Answering a research question with a single mean

**Research question is a generic form:** Is there evidence to support that the population mean is different than $\mu_0$?

-   $\mu_0$ is the generic form for a prescribed value: $\mu$ is the population mean and we want to see if $\mu=\mu_0$
...
::: green2-cont
-   Create a **confidence interval (CI)** for the population mean $\mu$ from our sample data and determine whether a prescribed value ($\mu_0$) is inside the CI or not
-   Answering the question: is $\mu_0$ a plausible value given our data?
    -   If CI does NOT contain $\mu_0$ then the answer is "Yes, evidence to support population mean is different than $\mu_0$
:::
:::::
::::::
...
::: pink-cont
-   Run a **hypothesis test** to see if there is evidence that the population mean $\mu$ is *significantly different* from a prescribed value ($\mu_0$)
...
::: {.column width="28%"}
Assuming the population mean is $\mu$ (65 inches), what is the probability that we observe $\overline{x}$ or a more extreme sample mean?
:::
::::::
...
## General steps in a Hypothesis Test
...
-   The assumptions to run a hypothesis test on a sample are:
    ...
    -   **Approximately normal sample or big n**: the distribution of the sample should be approximately normal, *or* the sample size should be at least 30
        -   AKA: sample is approximately Normal OR we can use the CLT
...
-   This is parallel to our confidence interval
    ...
    -   $\alpha$ is the probability of rejecting the null hypothesis when it is true (it's a measure of potential error)
        -   Null: our initial assumption about the mean (in example, $\mu = 98.6$)
