---
title: "Lesson 15: Power and sample size calculations for means"
...
-   [**Used this example to illustrate how to calculate a confidence interval and perform a hypothesis test for two independent samples**]{style="color:#5BAFF8"}
...
:::::::: columns
::: {.column width="48%"}
![](../img_slides/type-i-and-type-ii-errors_chart.png){fig-align="center" width="80%"}
...
-   a [null population]{style="color:#E75B5C"} with a normal distribution, centered at 0 with a standard deviation of 1 ($X_0 \sim Norm(0,1)$)
-   an [alternative population]{style="color:#3070BF"}, with normal dist'n, centered at 3 with standard deviation of 1 ($X_A \sim Norm(3,1)$)
...
two_sample_plot = ggplot(data.frame(x = c(-3, 6)), aes(x)) + 
  stat_function(fun = dnorm, 
                args = list(mean = mu1, sd = sd), 
                size = 1.5, 
                color = "#E75B5C") +
...
-   **Calculate** $n$
    ...
    -   Using calculated `d`: $$d = \frac{\overline{x}-\mu_0}{s}$$
