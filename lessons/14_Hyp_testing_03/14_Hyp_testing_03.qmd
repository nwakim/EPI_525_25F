---
title: "Lesson 14: Inference for difference in means from two independent samples"
...
1-pt(tstat, df=n1 + n2 - 2)
pt(tstat, df=n1 + n2 - 2, lower.tail = FALSE)

2*(1-pt(tstat, df=n1+n2 -2))
alpha <- 0.05
(p_area <- 1-alpha/2)

(tstar <- qt(p_area, df=n1+n2 -2 )) 
# (se <- sqrt(sd1^2/n1 + sd2^2/n2))
(moe <- tstar * se) 
(LB <- diff_x - moe)
(UB <- diff_x + moe)
...
::: {.column width="47%"}
Used $t^*$ = `qt(0.975, df=68)` = `r round(qt(0.975, df=68),3)`
...
```{r}
t.test(formula = Taps ~ Group, data = CaffTaps, var.equal = T)
```

```{r}
#| code-fold: true
#| code-summary: We can tidy the output
t_test_2samp = t.test(formula = Taps ~ Group, data = CaffTaps) %>% tidy()
t_test_2samp %>% gt() %>% 
  tab_options(table.font.size = 35) # use a different size in your HW
```

**Conclusion:**\
We are 95% confident that the difference in (population) mean finger taps/min between the caffeine and control groups is between `r round(t_test_2samp$conf.low, 3)` mg/dL and `r round(t_test_2samp$conf.high, 3)` mg/dL.
...
-   Statistical theory tells us that $t_{\overline{x}_1 - \overline{x}_2}$ follows a **Student's t-distribution** with $df = n_1 + n_2 - 2 = 68$
...
ggplot(data = data.frame(x = c(-4, 4)), aes(x)) + 
  stat_function(fun = dt, args = list(df = 68)) + 
  ylab("") + 
...
&= `r round(pt(tstat, df=68, lower.tail = FALSE), 8)`
\end{aligned}
$$
...
-   More realistic manuscript conclusion:
    -   The mean finger taps/min were `r xbar1` (SD = `r sd1`) and `r xbar2` (SD = `r sd2`) for the control and caffeine groups, and the increase of `r xbar1 - xbar2` taps/min was statistically discrenible ( $p$-value < 0.001).
