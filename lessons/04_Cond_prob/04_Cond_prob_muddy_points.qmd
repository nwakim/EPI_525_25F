...
### 1. Conditional Probability: How does it differ from marginal and joint probability?

For two events, here's a little summary table the different probabilities:

| Concept | Meaning | Formula | Example |
|----------------|------------------|------------------|--------------------|
| **Joint** | Both A and B occur | $P(A \cap B)$ | Smoker **and** has lung disease |
| **Marginal** | A occurs (overall) | $P(A)$ or $P(B)$ | Smoker (regardless of disease) |
| **Conditional** | A occurs **given** B occurred | $P(A \mid B) = P(A \cap B) / P(B)$ | Smoker **given** has lung disease |

### 2. I’m still a little fuzzy on determining when something is independent and when that is relevant to solving a problem Law of Total Probability??

The Law of Total Probability holds whether or not events are independent, so we do not need to determine if something is independent to use it. [Here's a video on how we construct the Law of Total Probability](https://www.youtube.com/watch?v=F3y8qupFfUs).

We will either be told that events are independent or we will need to prove that events are independent. **If we are told events are independent**, then we can use some of the special versions of our equations, like:

1.  $P(A \cap B) = P(A)P(B)$
2.  $P(A \mid B) = P(A)$

**If we do not know two events are independent**, then we need to show one of the above equations holds. You need to calculate both sides of the equation **using other rules** then see if they are equal.
...
[Please follow this link for my work through of the example.](https://ohsuitg-my.sharepoint.com/:v:/r/personal/wakim_ohsu_edu/Documents/Teaching/Classes/EPI_525_25F/Student_files_EPI_525/Recordings/04_Cond_prob_example.mp4?csf=1&web=1&nav=eyJyZWZlcnJhbEluZm8iOnsicmVmZXJyYWxBcHAiOiJPbmVEcml2ZUZvckJ1c2luZXNzIiwicmVmZXJyYWxBcHBQbGF0Zm9ybSI6IldlYiIsInJlZmVycmFsTW9kZSI6InZpZXciLCJyZWZlcnJhbFZpZXciOiJNeUZpbGVzTGlua0NvcHkifX0&e=lGXmAR)
