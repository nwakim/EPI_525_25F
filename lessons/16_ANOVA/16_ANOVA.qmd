---
title: "Lesson 16: Comparing Means with ANOVA"
...
-   We looked at inference for a **single mean** ($\mu$)
...
* How to measure variability __between__ groups? (aka how far apart are the means?)
* How to measure variability __within__ groups? (aka how much spread is there in each group?)
...
= \ \sum_{i = 1}^k n_i(\overline{x}_{i}-\overline{x})^2 \ \ 
...
* $\overline{x}_{i}$ = mean of observations in group _i_
* $\overline{x}$ = mean of _all_ observations
...
| Means       | $\overline{x}_{1}$ | $\overline{x}_{2}$ | $\overline{x}_{3}$ | $\ldots$ | $\overline{x}_{k}$ | $\overline{x}$ |
...
$$SST = \sum_{i = 1}^k \sum_{j = 1}^{n_i}(x_{ij} -\overline{x})^2 = (N-1)s^2$$

* where 
    * $N=\sum_{i=1}^{k}n_i$ is the total sample size and
    * $s^2$ is the grand standard deviation of all the observations

* This is the sum of the squared differences between each observed $x_{ij}$ value and the *grand mean*, $\overline{x}$. 
...
$$SSG = \sum_{i = 1}^k n_i(\overline{x}_{i}-\overline{x})^2$$

* This is the sum of the squared differences between each *group* mean, $\overline{x}_{i}$, and the *grand mean*, $\overline{x}$. 
...
$$SSE = \sum_{i = 1}^k\sum_{j = 1}^{n_i}(x_{ij}-\overline{x}_{i})^2 = \sum_{i = 1}^k(n_i-1)s_{i}^2$$
where $s_{i}$ is the standard deviation of the $i^{th}$ group

* This is the sum of the squared differences between each observed $x_{ij}$ value and its group mean $\overline{x}_{i}$. 
